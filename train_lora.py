...
class DataArguments:
    ...
    data_cache_path: str = field(
        default=None, metadata={"help": "Path to the cached jsonl data."}
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        input_ids, labels = tokenize(copy.deepcopy(item), self.tokenizer)
...
def load_json(data_path, data_cache_path):
    return datasets.load_dataset(
        "parquet", data_dir="/mnt/nfs/SFT-General/instruction_merge_set/data/"
    )
    if data_path.endswith(".json"):
        return json.load(open(data_path, "r"))
    elif data_path.endswith(".jsonl"):
        dataset = datasets.load_dataset(
            "json", data_files=data_path, cache_dir=data_cache_path
        )
